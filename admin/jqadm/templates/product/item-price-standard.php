...
$keys = [
	...
	'price.siteid', 'price.typeid', 'price.currencyid', 'price.status', 'price.quantity', 'price.taxrate', 'price.value', 'price.test', 'price.rebate', 'price.costs'
];
		...
		<div v-for="(entry, idx) in items" class="group-item card">
			...
			<div v-bind:id="'item-price-group-data-' + idx" v-bind:class="getCss(idx)"
				...
				<div class="col-xl-6">
                    ...
                    <div class="form-group row mandatory">
                        <label class="col-sm-4 form-control-label help"><?= $enc->html( $this->translate( 'admin', 'My new price' ) ); ?></label>
                        <div class="col-sm-8">
                            <input class="form-control item-taxrate" type="number" step="0.01" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
                                   v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'price.test' )
                                   ) ); ?>'.replace('idx', idx)"
                                   placeholder="<?= $enc->attr( $this->translate( 'admin', 'My new price' ) ); ?>"
                                   v-bind:readonly="checkSite('price.siteid', idx)"
                                   v-model="items[idx]['price.test']" />
                        </div>
                        <div class="col-sm-12 form-text text-muted help-text">
                            <?= $enc->html( $this->translate( 'admin', 'My new price' ) ); ?>
                        </div>
                    </div>
